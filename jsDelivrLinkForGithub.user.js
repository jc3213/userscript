...
var [none, username, repository, whatis] = components.splice(0, 4);
var jsdelivr = 'https://cdn.jsdelivr.net/gh/' + username + '/' + repository + '@';
var stylecss;
...
else if (pathname.includes('commit')) {
...
function createJSDelivrButton(menu) {
    ...
    copy.addEventListener('click', event => {
        if (whatis === 'blob') {
            copy.url = components.join('/');
        }
        else if (whatis === 'commit') {
            copy.url = [...components].pop() + '/' + menu.querySelector('a').title;
        }
        navigator.clipboard.writeText(jsdelivr + copy.url);
    });
