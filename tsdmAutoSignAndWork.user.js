...
var login = document.getElementById('lsform');
...
async function autoSign() {
    ...
    var iframe = await startWork('/plugin.php?id=dsu_paulsign:sign');
    var error = iframe.document.querySelector('#ct_shell > div:nth-child(1) > h1:nth-child(1)');
    if (error) {
...
async function autoWork() {
    ...
    if (iframe.document.querySelector('#messagetext')) {
        var text = iframe.document.querySelector('#messagetext > p:nth-child(1)').innerHTML.split(/<br>|<script/)[1];
